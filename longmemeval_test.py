"""
LongMemEval Test with MemU Memory Modules

This script mirrors locomo_test.py but adapts the input interface for LongMemEval:
- Each sample is a single question with a haystack of sessions.
- Sessions are lists of messages with role/content fields.
- Per-sample memory isolation (default) to avoid leakage across questions.
"""
...
class LongMemEvalTester:
    ...
    Workflow:
    1) Build memory from haystack sessions using MemAgent
    2) Answer question using ResponseAgent
    3) Evaluate answer using EvaluateAgent
    """
    ...
    def __init__(
        ...
        self.error_log_file = f"longmemeval_error_log_{self.log_timestamp}.txt"
        self._init_error_log()

        logger.info(
            "LongMemEval Tester initialized with MemAgent (memory) and ResponseAgent (QA)"
        )
    ...
    def _init_error_log(self):
        try:
            with open(self.error_log_file, "w", encoding="utf-8") as f:
                f.write(
                    f"LongMemEval Error Log - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                )
                f.write("=" * 80 + "\n")
    ...
    def _set_sample_memory_dir(self, sample_id: str):
        ...
        sample_dir = self.base_memory_dir / "longmemeval" / str(sample_id)
    ...
    def print_results(self):
        ...
        print("\n" + "=" * 60)
        print("LONGMEMEVAL TEST RESULTS - MEMU")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LongMemEval test with MemU")
    parser.add_argument(
        "--data-file",
    ...
    logger.info("Starting LongMemEval Test with MemU")
    results |= tester.run_test(args.data_file, args.sample_use)

    if results["success"]:
        tester.print_results()
        output_file = f"longmemeval_test_results_{tester.log_timestamp}.json"
